    ...
    if xPlayer then
        ...
        for _, mate in pairs(teamMates) do
            ...
            if fxTarget then
                ...
                TriggerClientEvent("0r-delivery:Client:OnTaskVehicleCreated", mateSource, netId, plate, coords)
            end
        end
    end
end
...
function Koci.Server.Functions.StartNewTaskDestination(source)
    ...
    if xPlayer then
        if not xPlayer.PlayerData.onDelivery then
            ...
            TriggerClientEvent("0r-delivery:Client:StartNewTaskDestination",
                source,
                Destination,
                pedModel
            )
...
function Koci.Server.Functions.StartTaskWithTeamMates(source, taskId)
    ...
    if xPlayer then
        ...
        if Task then
            ...
            TriggerClientEvent("0r-delivery:Client:StartTask", source, taskId, true)
            local teamMates    = Koci.Utils.deepCopy(xPlayer.PlayerData.TeamMate or {})
            ...
            for _, mate in pairs(teamMates) do
                ...
                if fxTarget then
                    ...
                    Koci.Server.Players[fxTarget.PlayerData.identifier] = fxTarget
                    TriggerClientEvent("0r-delivery:Client:StartTask", mateSource, taskId, false)
                end
            end
        end
    end
    ...
    if xPlayer then
        ...
        for _, mate in pairs(teamMates) do
            ...
            if fxTarget then
                ...
                Koci.Server.Players[fxTarget.PlayerData.identifier] = fxTarget
                TriggerClientEvent("0r-delivery:Client:OnCargoPutOnTaskVehicle",
                    mateSource,
                    source
                )
            end
    ...
    if xPlayer then
        ...
        for _, mate in pairs(teamMates) do
            ...
            if fxTarget then
                ...
                fxTarget.PlayerData.numberOfCargoInVehicle - 1
                Koci.Server.Players[fxTarget.PlayerData.identifier] = fxTarget
                TriggerClientEvent("0r-delivery:Client:OnCargoPickUpFromTaskVehicle",
                    mateSource,
                    source
                )
            end
...
function Koci.Server.Functions.OnCargoPickUpFromDestinationPed(source)
    ...
    if xPlayer then
        ...
        for _, mate in pairs(teamMates) do
            ...
            if fxTarget then
                TriggerClientEvent("0r-delivery:Client:OnCargoPickUpFromDestinationPed",
                    mateSource,
                    source
                )
            end
...
function Koci.Server.Functions.HandOverTaskVehicle(source)
    ...
    if xPlayer then
        local teamMates = Koci.Utils.deepCopy(xPlayer.PlayerData.TeamMate or {})
        TriggerClientEvent("0r-delivery:Client:HandOverTaskVehicle", source)
        for _, mate in pairs(teamMates) do
            local mateSource = mate.source
            local fxTarget = Koci.Server.Functions.GetPlayer(mateSource)
            if fxTarget then
                TriggerClientEvent("0r-delivery:Client:HandOverTaskVehicle", mateSource)
            end
        end
    end
end
...
function Koci.Server.Functions.OnTaskDestionCompleted(source, prop)
    ...
    if xPlayer then
        ...
        xPlayer.PlayerData.onDelivery = false
        xPlayer.PlayerData.currentDestination = nil
        TriggerClientEvent("0r-delivery:Client:OnTaskDestionCompleted",
            source,
            prop
        )
        local teamMates = Koci.Utils.deepCopy(xPlayer.PlayerData.TeamMate or {})
    ...
    if xPlayer then
        ...
        CreateThread(function()
            Koci.Server.ExecuteSQLQuery(
                "UPDATE `0resmon_delivery_employees` SET profile = ?, level = ?, exp = ? WHERE user = ?",
                {
...
function Koci.Server.Functions.OnTaskCompleted(source)
    ...
    if xPlayer then
        ...
        TriggerClientEvent("0r-delivery:Client:OnTaskCompleted", source, newLevel, newExp)
        ...
        for _, mate in pairs(teamMates) do
            ...
            if fxTarget then
                ...
                Koci.Server.Players[fxTarget.PlayerData.identifier] = fxTarget
                TriggerClientEvent("0r-delivery:Client:OnTaskCompleted", mateSource, newLevel, newExp)
